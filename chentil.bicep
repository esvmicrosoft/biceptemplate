...
var machines = [
  { publisher: 'redhat', offer: 'rhel', sku: '97-gen2',  version: 'latest' }
]
...
resource networkInterface 'Microsoft.Network/networkInterfaces@2023-09-01' = [for i in range(0,maxunits): {
...
resource publicIPAddress 'Microsoft.Network/publicIPAddresses@2023-09-01' = [for i in range(0,maxunits): {
...
resource vm 'Microsoft.Compute/virtualMachines@2023-09-01' = [for i in range(0,maxunits): {
  name: '${vmName}${i}'
  location: location
  properties: {
    hardwareProfile: {
...
resource vm_custom_script 'Microsoft.Compute/virtualMachines/extensions@2015-06-15' = [ for i in range(0, maxunits): {
  parent: vm[i]
  ...
  properties: {
    ...
    protectedSettings: {
      commandToExecute: 'date > /dev/console'
    }
  }
}]
